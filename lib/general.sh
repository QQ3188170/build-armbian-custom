...
#
# <release>: buster|bullseye|bionic|focal|hirsute|jammy|impish|sid
...
create_sources_list()
{
	...
	xenial|bionic|focal|hirsute|jammy|impish)
	cat <<-EOF > "${basedir}"/etc/apt/sources.list
...
# function: cycle trough distributions
	...
	local distributions=("stretch" "bionic" "buster" "bullseye" "focal" "hirsute" "jammy" "impish" "sid")
...
# build aarch64
  if [[ $(dpkg --print-architecture) == amd64 ]]; then
	...
	if [[ -z $HOSTRELEASE || $HOSTRELEASE =~ ^(focal|debbie|buster|bullseye|impish|hirsute|jammy|ulyana|ulyssa|uma)$ ]]; then
